...
int Packet_Filtering(uint8_t *buff_up, int buff_index);	//LSK 추가
    ...
    *(uint32_t *)(buff_up + 8) = net_mac_l;
    ...
    while (!exit_sig && !quit_sig) {
        ...
        buff_up[buff_index] = 0; /* add string terminator, for safety */
				...
				/* LSK 추가 */
				if(0 == Packet_Filtering(buff_up, buff_index))	continue;
...
int Packet_Filtering(uint8_t *buff_up, int buff_index)
{
    /* LSK 추가 */
    int sock_up_Flag = 1;

    char tempBuff[buff_index];
    if(0 < buff_index) {
        memcpy((void *)tempBuff, (void *)(buff_up + 12), buff_index);
    }

    int sizeVal = 0;
    char* sizeStr = strstr(tempBuff, "size");
    if(NULL != sizeStr) {
        sizeVal = atoi(sizeStr + 6);
        printf("** LSK size : %d\n", sizeVal);
    }

    char* dataFullStr = strstr(tempBuff, "data");
    if(NULL != dataFullStr) {
        printf("** LSK raw  : %s\n", dataFullStr);
...
//				if(sizeVal < 50) {
					...
					int b64_to_bin_Flag = 0;
					int j = b64_to_bin(data, strlen(data), binStr, sizeVal);
					if(j == -1)	{
						printf("** LSK Hex  : b64_to_bin fail\r\n");
					} else {
						printf("** LSK Hex  : ");
					...
					if(b64_to_bin_Flag) {
						//header 00 - join req filter : Hex Value로 비교
						if(0x00 == binStr[0]){
							if((0x78 == binStr[1]) || (0xff == binStr[1])) {
								printf("  ++++++++++++++++++++++++++++++++++++++++++++++++ LSK HS_Join_Req !!!!!\n");
							} else {
								printf("  ------------------------------------------------ LSK Othrer Join_Req (filter) !!!!!\n");
							}
						}
					} else {
		        //header 00 - join req filter : Ascii 문자열로 비교
		       	if(0 == strncmp(dataFullStr + 7, "A", 1)) {
		            char* chkIdx = strstr(dataFullStr, "AHgiMVZygDcO");
		            char* chkIdx2 = strstr(dataFullStr, "AP8iMVZygDcO");
		            if((NULL != chkIdx) || (NULL != chkIdx2)) {
			            printf("  ++++++++++++++++++++++++++++++++++++++++++++++++ LSK HS_Join_Req !!!!!\n");
		            } else {
									sock_up_Flag = 0;
			            printf("  ------------------------------------------------ LSK Othrer Join_Req (filter) !!!!!\n");
