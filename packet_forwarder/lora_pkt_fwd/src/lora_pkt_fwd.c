...
void thread_up(void) {
    ...
    *(uint32_t *)(buff_up + 8) = net_mac_l;
    ...
    while (!exit_sig && !quit_sig) {
        ...
        char* dataStr = strstr(tempBuff, "data");
				if(NULL != dataStr) {
	        printf("** LSK data : %s\n", dataStr);

					if(sizeVal < 50) {
						//header 00 - join req
						if(0 == strncmp(dataStr + 7, "A", 1)) {
		          char* chkIdx = strstr(dataStr, "AHgiMVZygDcO");
		          char* chkIdx2 = strstr(dataStr, "AP8iMVZygDcO");
